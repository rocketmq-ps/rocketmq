...
public class BrokerOuterAPI {
    ...
    public TopicList getTopicListFromNameServer(long timeoutMs) throws RemotingException, MQClientException, InterruptedException {
        RemotingCommand request = RemotingCommand.createRequestCommand(RequestCode.GET_ALL_TOPIC_LIST_FROM_NAMESERVER, null);

        RemotingCommand response = this.remotingClient.invokeSync(null, request, timeoutMs);
        assert response != null;
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                byte[] body = response.getBody();
    ...
    public ClusterInfo getBrokerClusterInfo(long timeoutMs) throws RemotingException, MQClientException, InterruptedException {
        ...
        switch (response.getCode()) {
            case ResponseCode.SUCCESS: {
                byte[] body = response.getBody();
                if (body != null) {
                    return TopicList.decode(body, ClusterInfo.class);
                }
            }
            default:
                break;
